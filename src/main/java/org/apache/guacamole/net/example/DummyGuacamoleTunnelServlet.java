...
public class DummyGuacamoleTunnelServlet extends GuacamoleHTTPTunnelServlet {
	...
	protected GuacamoleTunnel doConnect(HttpServletRequest request) throws GuacamoleException {
		...
		String guacdClientHost = request.getParameter("guacd-ip");
		
		String browserHost = request.getParameter("browser-ip");

		// guacd connection information
		String hostname = guacdClientHost;
		int port = 4822;

		// VNC connection information
		GuacamoleConfiguration config = new GuacamoleConfiguration();
		config.setProtocol("vnc");
		config.setParameter("hostname", browserHost);
